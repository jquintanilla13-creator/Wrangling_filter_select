...
After running the code, we found that there are 8 Batman movies in the data.
...
We believe the second code is incorrect and does not run because it is in the wrong order. 
...
After running the codes, we found that th longest movie in terms of length was Cure For Insomnia with 5220 minutes. The oldest movie in terms of year was Blacksmith Scene which was made in 1893.
...
After running the code above, we found that 1248 movies were made in 1995. 
...
After running the code above, we found that there are four movies that fall under those categories.
...
After running the code above, we found that there are three movies of Harry Potter included (The Chamber of Secrets, the prisoner of Azkaban and the Sorcerer's Stone). The movie with the highest rating was The Prisoner of Azkaban with a 7.8 rating, while the movie with the highest votes was the Sorcerer's Stone with 46741 votes.
...
After running the code above, we found that the highest budget for a comedy or animation movie goes to Wild Wild West with a budget of $170000000.
